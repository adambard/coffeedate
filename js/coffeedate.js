...
(function() {
  var CoffeeDate, DAYS, FORMAT_MAPPINGS, MONTHS, PARSE_MAPPINGS, fmt_month_name, fmt_weekday_name, parse_month_abbr, parse_noop, weekday_num, year_parser, zeropad;
  ...
  year_parser = function(base, pivot) {
    return [
      /^[0-9]{1,2}/, function(s, d) {
        ...
        yr = parseInt(s, 10);
        if (yr > pivot) {
          return d.year = base + yr;
        } else {
          return d.year = base + 100 + yr;
        }
      }
    ];
  };
  ...
  CoffeeDate = (function() {
    CoffeeDate.set_parser = function(fmt, spec) {
      return PARSE_MAPPINGS[fmt] = spec;
    };

    CoffeeDate.year_parser = year_parser;
